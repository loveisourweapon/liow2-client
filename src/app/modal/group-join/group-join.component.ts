...
export class GroupJoinModalComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.stateSubscription = this.state.modal.groupJoin$.subscribe((state: ModalState) => {
    ...
    this.searchResults$ = this.searchQuery$.debounceTime(200).switchMap((searchQuery: string) =>
      Observable.if(
        () => searchQuery.length < 2,
        Observable.of([]),
        this.groupService
          .find({ query: searchQuery, fields: '_id,name,urlName' })
        ...
        Observable.if(
          ...
          this.state.auth.user$
            ...
            .switchMap((authUser: User) =>
              Observable.if(
                () =>
                  findIndex(authUser.groups, (authGroup: Group) => authGroup._id === group._id) >=
                  0,

                // Already member of group, notify and close
                Observable.of(true).do(() =>
                  this.alertify.log(`Already a member of group <b>${group.name}</b>`)
                ),
