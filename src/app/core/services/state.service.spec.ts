...
describe(`StateService`, () => {
  ...
  describe(`auth`, () => {
    ...
    it(`should notify next group$ value when group value is set`, () => {
      ...
      service.auth.group$.first().subscribe((group: Group) => expect(group).toBeNull());
      service.auth.group = testGroup;
      service.auth.group$.first().subscribe((group: Group) => expect(group).toBe(testGroup));
    });

    it(`should notify next campaign$ value when campaign value is set`, () => {
      const testCampaign = <Campaign>{};

      service.auth.campaign$.first().subscribe((campaign: Campaign) => expect(campaign).toBeNull());
      service.auth.campaign = testCampaign;
      service.auth.campaign$
        .first()
    ...
    it(`should notify next user$ value when user value is set`, () => {
      ...
      service.auth.user$.first().subscribe((user: User) => expect(user).toBeNull());
      service.auth.user = testUser;
      service.auth.user$.first().subscribe((user: User) => expect(user).toBe(testUser));
    });
  });
  ...
  describe(`controlPanel`, () => {
    ...
    it(`should notify next deeds$ value when deeds value is set`, () => {
      ...
      service.controlPanel.deeds$.first().subscribe((deeds: Deed[]) => expect(deeds).toEqual([]));
      service.controlPanel.deeds = testDeeds;
      service.controlPanel.deeds$
        .first()
    ...
    it(`should notify next group$ value when group value is set`, () => {
      ...
      service.controlPanel.group$.first().subscribe((group: Group) => expect(group).toBeNull());
      service.controlPanel.group = testGroup;
      service.controlPanel.group$
        .first()
    ...
    it(`should notify next user$ value when user value is set`, () => {
      ...
      service.controlPanel.user$.first().subscribe((user: User) => expect(user).toBeNull());
      service.controlPanel.user = testUser;
      service.controlPanel.user$.first().subscribe((user: User) => expect(user).toBe(testUser));
    });

    it(`should notify next users$ value when users value is set`, () => {
      const testUsers = [<User>{ _id: 'abc123' }];

      service.controlPanel.users$.first().subscribe((users: User[]) => expect(users).toEqual([]));
      service.controlPanel.users = testUsers;
      service.controlPanel.users$
        .first()
  ...
  describe(`deed`, () => {
    it(`should notify next deed$ value when deed value is set`, () => {
      ...
      service.deed$.first().subscribe((deed: Deed) => expect(deed).toBeNull());
      service.deed = testDeed;
      service.deed$.first().subscribe((deed: Deed) => expect(deed).toBe(testDeed));
    });
  });
  ...
  describe(`group`, () => {
    it(`should notify next group$ value when group value is set`, () => {
      ...
      service.group$.first().subscribe((group: Group) => expect(group).toBeNull());
      service.group = testGroup;
      service.group$.first().subscribe((group: Group) => expect(group).toBe(testGroup));
    });

    it(`should notify next campaign$ value when campaign value is set`, () => {
      const testCampaign = <Campaign>{};

      service.campaign$.first().subscribe((campaign: Campaign) => expect(campaign).toBeNull());
